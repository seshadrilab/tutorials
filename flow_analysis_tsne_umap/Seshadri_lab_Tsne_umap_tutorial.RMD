...
t-distributed stochastic neighbor embedding (t-SNE) is a non-linear dimensionality reduction statistical analysis useful for assessing high-dimensional data. This is achieved by giving each data point withing a data set a position in a 2 or 3 dimensional map. Specifically, it models each high-dimensional object by a two- or three-dimensional point in such a way that similar objects are modeled by nearby points and dissimilar objects are modeled by distant points with high probability. In flow cytometry, t-SNE analyses are useful for segregating and identifying unique cellular porpulations based on their surface protein expression as well as the cytokines or chemokines being secreted by specific subsets.

For this tutorial, we will be analyzing a batch of data from the HAARVI cohort. The aim of this tutorial is to enable you to read in, QC, analyse, and visualize surface staining and intracellular staining cell data from a flow cytometry experiment.

NOTE: If you have successfully completed the COMPASS tutorial, the same saved gating set can be used allowing you to skip the initial reading-in and QC of the raw FACS files.
...
With the data read-in, QC'ed and converted into a gatingset, it is time to extract the data required for downstream t-SNE.
...
In some instances, it may be prudent to sample your data so that each condition (i.e. "Stim"ulation) has an equal number of cells or observations. This can prevent potential bias; however, it may not be necessary for all analyses. Here, the CD4 cell population was selected and each "Stim" sampled 5000 times (i.e. 5000 cells per "Stim" per patient).
...
### Create a data matrix to input into t-SNE
...
Next, assess whether the defined populations (gates_to_extract_membership) exist in the gating set. This is a useful QC step at this stage of the analysis. The result should be returned as TRUE.
...
Extract the CD4 specific data and input it into the matrix.
...
Scaling centers and/or scales the columns of a numeric matrix by the normalization of a dataset using the mean value and standard deviation and is a standard step in all
...
The following steps may take significant amount of time depending on the the number of variables, computational power, and number of cells sampled. Further, the number of iterations can also be altered.


### Set the seed

A seed is a necessary numerical marker used by t-SNE analyeses to ensure reproducibility. If not defined, the t-SNE output may vary from run to run.Below we have used the date on which the following tutorial was created; however, this can be any value you want.  
```{r}
set.seed(date)
```
...
It is interesting to overlay ICS or SS data onto the t-SNE plot we generated above.The following chunk of code defines a set theme from which the proceeding plots are generated. This can be easily altered to your individual needs.
...
Below, is an example whereby TNFa expression is overlaid onto the CD4 t-SNE data and then stratified (as.factor(x)) by stimulation condition.
